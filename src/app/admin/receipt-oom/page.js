...
const ReceiptScreen = ({ region = 2 }) => {
  ...
  const createReceipt = async (receiptData) => {
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data?.error || 'Failed to create receipt');
  ...
  useEffect(() => {
    if (userViewDetail?.rate_r2 !== undefined && userViewDetail?.rate_r2 !== null) {
      setAmountPerShare(Math.ceil(parseFloat(userViewDetail.rate_r2) || 0));
    }
    if (selectedRowData) {
